...
elseif ($Action -eq "recover") {
    ...
    foreach ($pc in $TargetPCs) {
        ...
        try {
            # Step 1: Kill the watchdog FIRST via taskkill (not PowerShell)
            # This prevents the watchdog from killing our recovery PowerShell process
            $killArgs = @("\\$pc") + $CredArgs + @(
                "-s", "-h", "-accepteula",
                "taskkill.exe", "/f", "/im", "wscript.exe"
            )
            & $PsExec @killArgs 2>&1 | Out-Null

            # Step 2: Also kill the HTA
            $killArgs2 = @("\\$pc") + $CredArgs + @(
                "-s", "-h", "-accepteula",
                "taskkill.exe", "/f", "/im", "mshta.exe"
            )
            & $PsExec @killArgs2 2>&1 | Out-Null

            # Step 3: Now run recovery script safely (watchdog is dead)
            $remotePath = "\\$pc\C$\CyberExercise"
            if (!(Test-Path $remotePath)) { New-Item -ItemType Directory -Path $remotePath -Force | Out-Null }
            ...
            & $PsExec @psexecArgs 2>&1 | Out-Null
            Write-Host "RECOVERED — student must log off and back in" -ForegroundColor Green
        } catch {
            Write-Host "FAILED — needs WinRE manual recovery" -ForegroundColor Red
        }
    }
    Write-Host "`n  NOTE: Students need to log off (Ctrl+Alt+Del) and log back in to get their desktop." -ForegroundColor Yellow
}
