...
| `.\deploy_all.ps1 -Action recover -Username "Admin" -Password "Pass"` | Undo all (students must log off/on after) |

---

## How the Trigger Works

**Option A — Manual trigger at 14:30 (RECOMMENDED):**
Run `.\deploy_all.ps1 -Action trigger` at 14:30. Uses PsExec interactive mode so the HTA appears directly on the student's screen. Instant red screen.

**Option B — Scheduled task (automatic):**
The deploy command creates a scheduled task for 14:30. Runs as SYSTEM so the HTA can't show directly on screen. Instead: registry gets locked, explorer dies, watchdog starts, then the student gets forced logged off. When they log back in, the shell replacement shows the HTA. Takes about 10 seconds.

**Recommendation:** Deploy in the morning to pre-stage files and create the scheduled task as backup. At 14:30, use `-Action trigger` manually for instant effect.
...
| After remote recovery, still black/HTA screen | Registry is fixed but Explorer runs in SYSTEM session. Student does Ctrl+Alt+Del -> Sign out, then logs back in normally |
